...
${u}`, c = n.createShaderModule({ code: d, label: e.name });
  ...
  // node_modules/demucs/dist/onnx-htdemucs.js
  ye.wasm.wasmPaths = "./";
  var ONNXHTDemucs = class _ONNXHTDemucs {
    ...
    static async init(modelWeights) {
      let instance = new _ONNXHTDemucs();
      let opts = typeof self === "undefined" || typeof self.location === "undefined" ? void 0 : { executionProviders: ["webgpu", "wasm"] };
      let session = await vf.create(modelWeights, opts);
    ...
    async forward(mix, magspec) {
      const mixTensor = new Ke("float32", mix.data, mix.shape);
      const magspecTensor = new Ke("float32", magspec.data, magspec.shape);
      const feeds = {};
  ...
  // node_modules/demucs/dist/dsp.js
  ...
  function pad1d(x, paddings, mode = "constant") {
    ...
    if (mode === "reflect") {
      ...
      if (length <= maxPad) {
        ...
        const shape2 = [...xShape];
        shape2[shape2.length - 1] = length + extraPadLeft + extraPadRight;
        const totalSize2 = shape2.reduce((a, b) => a * b, 1);
        const tempData = new Float32Array(totalSize2);
        const outerSize2 = totalSize2 / shape2[shape2.length - 1];
        const newLength = shape2[shape2.length - 1];
        for (let i = 0; i < outerSize2; i++) {
  ...
  function stft(x, nFft, hopLength, window, normalized = true, center = true, padMode = "reflect") {
    ...
    const norm = normalized ? 1 / Math.sqrt(nFft) : 1;
  ...
  function spec(x) {
    ...
    const le = Math.ceil(lastDimLength / hl2);
    const pad = Math.floor(hl2 / 2) * 3;
    const paddingRight = pad + le * hl2 - lastDimLength;
    const paddedX = pad1d(x, [pad, paddingRight], "reflect");
    const z = spectro(paddedX, 4096, hl2);
  ...
  function magnitude(z) {
    const [B, C, Fr2, T] = z.real.shape;
    const outShape = [B, C * 2, Fr2, T];
    const outSize = B * C * 2 * Fr2 * T;
    const out = new Float32Array(outSize);
    for (let b = 0; b < B; b++) {
      for (let c = 0; c < C; c++) {
        for (let fr = 0; fr < Fr2; fr++) {
          for (let t = 0; t < T; t++) {
            const srcIdx = b * C * Fr2 * T + c * Fr2 * T + fr * T + t;
            const dstIdxReal = b * (C * 2) * Fr2 * T + c * 2 * Fr2 * T + fr * T + t;
            out[dstIdxReal] = z.real.data[srcIdx];
            const dstIdxImag = b * (C * 2) * Fr2 * T + (c * 2 + 1) * Fr2 * T + fr * T + t;
  ...
  function istft(z, nFft, hopLength, window, normalized = true, length = null, center = true) {
    ...
    const norm = normalized ? Math.sqrt(nFft) : 1;
    for (let b = 0; b < batch; b++) {
      for (let frame = 0; frame < numFrames; frame++) {
  ...
  function ispec(z, length) {
    ...
    const pad = Math.floor(hl2 / 2) * 3;
    const le = hl2 * Math.ceil(length / hl2) + 2 * pad;
    const x = ispectro(paddedZ, hl2, le);
    const shape = x.shape;
  ...
  // node_modules/demucs/dist/wav-utils.js
  function samplesToWav(channelData, sampleRate = 44100, asFloat = false) {
    const channels = channelData.length;
    const samples = channelData[0].length;
    const bitsPerSample = asFloat ? 32 : 16;
    const bytesPerSample = bitsPerSample / 8;
    const audioFormat = asFloat ? 3 : 1;
    const dataSize = samples * channels * bytesPerSample;
    const fileSize = 36 + dataSize;
    const buffer = new Uint8Array(44 + dataSize);
    const view = new DataView(buffer.buffer);
    let offset = 0;
    const writeString = (str, offset2) => {
      for (let i = 0; i < str.length; i++) {
        buffer[offset2 + i] = str.charCodeAt(i);
      }
    };
  ...
  function planarize(channelData) {
    ...
    if (channels === 1) {
      ...
      for (let c = 1; c < channels; c++) {
        if (channelData[c].buffer !== channelData[0].buffer || channelData[c].byteOffset !== channelData[c - 1].byteOffset + channelData[c - 1].length * 4) {
          isSequential = false;
          break;
        }
      }
  ...
  // node_modules/demucs/dist/apply.js
  var TensorChunk = class {
    tensor;
    offset;
    length;
  ...
  function mask(m) {
    const [B, S, C, Fr2, T] = m.shape;
    const C_half = C / 2;
    const outShape = [B, S, C_half, Fr2, T];
        ...
        for (let c = 0; c < C_half; c++) {
          for (let fr = 0; fr < Fr2; fr++) {
            for (let t = 0; t < T; t++) {
              ...
              const realInIdx = b * S * C * Fr2 * T + s * C * Fr2 * T + realChannel * Fr2 * T + fr * T + t;
              const imagInIdx = b * S * C * Fr2 * T + s * C * Fr2 * T + imagChannel * Fr2 * T + fr * T + t;
              const outIdx = b * S * C_half * Fr2 * T + s * C_half * Fr2 * T + c * Fr2 * T + fr * T + t;
  ...
  async function separateTracks(model, rawAudio, progressCallback, overlap = 0.25) {
    ...
    for (let s = 0; s < sources; s++) {
      ...
      for (let c = 0; c < channels; c++) {
        ...
        const channelArray = new Float32Array(
          result.data.buffer,
          startIdx * 4,
          // byte offset (Float32 = 4 bytes)
          length
        );
        channelData2.push(channelArray);
      }
      tracks[sourceName] = { channelData: channelData2, sampleRate };
    }
    return tracks;
  }
